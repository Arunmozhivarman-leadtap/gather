...
export default function SubscriptionForm() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const result = await subscribeAction(formData, token);
    ...
    return (
      <div className="bg-brand-green/20 backdrop-blur-md p-8 rounded-3xl border border-brand-green/30 animate-in zoom-in-95 duration-500">
        <div className="flex flex-col items-center gap-4 text-center">
          ...
          <h3 className="text-2xl font-serif text-brand-linen">
            You&apos;re on the list!
          </h3>
  ...
  return (
    <form
      ...
      className="max-w-lg mx-auto space-y-6 animate-in fade-in duration-500 delay-300"
    >
      <div className="flex flex-col md:flex-row gap-3">
        ...
        <div className="relative md:w-1/3">
          ...
          >
            <option className="text-brand-dark" value="Find Gathering">
              Find Gathering
            </option>
            <option className="text-brand-dark" value="Create Gathering">
              Create Gathering
            </option>
            <option className="text-brand-dark" value="Share a place or Venue">
              Share a place or Venue
            </option>
            <option className="text-brand-dark" value="All of the above">
              All of the above
            </option>
            <option className="text-brand-dark" value="Just Curious">
              Just Curious
            </option>
          </select>
      ...
      {error && (
        <p className="text-brand-terracotta text-sm text-center font-medium bg-white/10 p-2 rounded-lg">
          {error}
        </p>
      )}
